...
The bundled demo uses a Markdown Knowledge workspace under `examples/demo-workspace` and a separate Agent fixture under `examples/demo-execution-workspace`. It represents Gareji Board, Gareji Core, and the Zettelkasten plugin as three independent product projects without copying personal notes or source repositories into the demo. Each additional plugin becomes its own product project and can be connected to its actual local repository as an execution workspace.

## Planned first launch

The first launcher exposes two choices:

- **Open sample** creates a resettable, disposable session from the bundled Knowledge and Execution workspace fixtures, including sample Agent instructions and Skills. It never writes into the bundled fixtures themselves.
...
The sample path is `gareji-board demo`; use `gareji-board demo --reset` to restore the copied fixture state first. From a source checkout, run `cargo run -p gareji-board-bridge --bin gareji-board -- demo --reset`. The launcher always uses its isolated application-owned demo database, copies both bundled workspaces, and opens the desktop Board without reading `GAREJI_BOARD_DB`. See [the architecture](docs/architecture.md) and [the onboarding design](docs/onboarding-design.md) for responsibility, connection, and trust rules.
...
Run the isolated sample with `cargo run -p gareji-board-bridge --bin gareji-board -- demo --reset`, or run the normal Rust desktop shell with `cargo run -p gareji-board-app`. The demo launcher copies the bundled Knowledge and Execution workspaces into an application-owned session, seeds three projects and their built-in graph definitions, connects the copied Execution workspace, and opens the app with the demo database selected. The normal shell uses the configured local Board database and loads recent Progress Checkpoints through a local `gareji-core bridge` when Core is installed. Project-only Checkpoints appear in the Activity Inbox and can be attached to an existing Work item or used to create and attach a new `todo` Work item without rewriting Core history. Linked state recommendations can then be accepted or dismissed from the Activity timeline; accepted supported recommendations update Board state transactionally. The project card shows whether its connected Execution workspace is available and whether it belongs to a local Git repository. The Agent catalog creates and edits scheduling roles, declared capabilities, portable instruction-file references, and stable Skill references, then performs a presence-only inspection against the Execution workspace connected to the selected Board project. Connect or replace that local workspace from its project card; this does not enable or trust its Skills. The Work item control surface groups individual items into canonical-state Kanban lanes, applies explicit human state changes, and edits each item's Agent plan without letting a stale screen overwrite newer Board state. The Safe Autopilot button previews one deterministic candidate and its skip reasons; its explicit start action then runs that Work item's current Agent Loop in a background isolated worktree. Completion is recorded through Core and can advance one declared Graph route without changing Work item state. Gate and Audit stages require a typed evidence reference, Approval stages require a separate explicit human confirmation, and Terminal stages cannot start another Runner. Each Project card exposes its immutable Route decision history with the selected path and evidence references. It can also propose replacing one Agent Loop profile as a candidate Graph revision; approval publishes it for future work while already active work stays pinned, and rejection changes nothing. `GAREJI_BOARD_DB`, `GAREJI_CORE_BIN`, `GAREJI_CORE_DB`, `GAREJI_CODEX_BIN`, and `GAREJI_RUN_ROOT` can select explicit normal local state, Core, Codex, and Run storage. `cargo run -p gareji-board-bridge --bin gareji-board -- seed-sample` initializes the normal database explicitly for Core integration. Core capability composition remains pending.

Run one due Portfolio scheduling pass with `cargo run -p gareji-board-bridge --bin gareji-board -- portfolio-tick-due`. On Windows, `powershell -ExecutionPolicy Bypass -File scripts/install-portfolio-scheduler.ps1` builds the release CLI and registers a five-minute current-user task, so enabled Portfolio schedules continue while the desktop is closed. The command advances at most one node per due revision, never starts a project Runner, and leaves Approval nodes waiting for a person.
