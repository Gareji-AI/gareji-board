...
The desktop now includes visual Control Graph, Orchestration Blueprint, and Portfolio orchestration editors backed by reversible Draft Modules and immutable candidate revisions. Portfolio Runs and their steps are durable. Enabled interval schedules use the same finite `PortfolioScheduler` Module from the open desktop or the one-shot local CLI; an OS task can wake that CLI while the desktop is closed without adding a Board daemon. Competing wake-ups compare the latest Run transactionally, and scheduled Portfolio ticks never start a project Runner or bypass a waiting human approval.
