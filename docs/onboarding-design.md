...
`gareji-board demo` creates or reopens a disposable session from fixtures embedded in the CLI, then opens the Board. `gareji-board demo --reset` restores the copied fixture state before opening, and `gareji-board demo --prepare-only` prepares and validates the session without opening a window. From a source checkout, prefix those arguments with `cargo run -p gareji-board-bridge --bin gareji-board --`. The session includes three explicitly labeled sample projects (`Gareji Board · Sample`, `Gareji Core · Sample`, and `Sample Knowledge Plugin`), four agent profiles, four inspectable instruction files, four inspectable Skills, project-to-graph bindings, safe autopilot settings, and example success and blocked outcomes.

The launcher copies writable demo material into an application-owned `demo-session` directory. The desktop header says `Demo workspace`, every Project card carries a `SAMPLE` badge, and existing-project connection controls are hidden for this session. Knowledge notes come from the copied demo Knowledge workspace; Agent instructions, Work item acceptance criteria, and Skills come from the copied demo Execution workspace. That copy is initialized as its own local Git repository so every live Runner start can create an isolated worktree without touching a source checkout. The launcher uses `demo-session/board.sqlite3` even when `GAREJI_BOARD_DB` selects a normal Board database. When `gareji-core` is available, it registers only the sample projects in `demo-session/core.sqlite3` and records the seeded Activity there. It only removes an existing session during `--reset` after verifying its private marker, and never installs a product plugin or writes into either bundled fixture, a normal Core database, a personal knowledge workspace, or an existing source repository.

Browsing the seeded sample does not require an account. Starting the explicit live Agent Loop requires a locally installed, authenticated Codex CLI; the launcher resolves its native executable and passes it to the desktop app. The resulting Run evidence and worktree remain below application-owned local Run storage.
